...
## Filter and only show the significant pathways
for (i in 1:length(Cluser))
  ...
  gostres.up <- gost(query = Up.DEG, 
                     ...
                     numeric_ns = "", sources = c("GO:BP", "KEGG"),
                     as_short_link = FALSE)
  ...
  write.csv(Result.up, paste0("Step7.Pathway/GO/Go.up.", Cluser[i], ".csv"))
  write.csv(Result.down, paste0("Step7.Pathway/GO/Go.down.", Cluser[i], ".csv"))}, 
           error = function(e){})
...
for (i in 1:length(Cluser))
  ...
  gostres.up <- gost(query = Up.DEG, 
                     ...
                     multi_query = FALSE, significant = TRUE, exclude_iea = FALSE, 
                     measure_underrepresentation = FALSE, evcodes = TRUE, 
                     user_threshold = 1.0, correction_method = "g_SCS", 
                     domain_scope = "annotated", custom_bg = NULL, 
                     numeric_ns = "", sources = c("GO:BP", "KEGG"),
                     as_short_link = FALSE)
  Result.up <- apply(gostres.up$result, 2, as.character) %>% as.data.frame()
  ...
  write.csv(Result.up, paste0("Step7.Pathway/GO/Go.up.Complete.", Cluser[i], ".csv"))
  write.csv(Result.down, paste0("Step7.Pathway/GO/Go.down.Complete.", Cluser[i], ".csv"))}, 
  error = function(e){})
