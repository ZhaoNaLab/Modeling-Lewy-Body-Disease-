...
reg <- makeExperimentRegistry("ReclusterMajorCluster.R2.Registry",
                              packages = c("tidyverse", "Seurat", "data.table", "harmony"), 
                              seed = 20221005)

reg <- loadRegistry("ReclusterMajorCluster.R2.Registry", writeable = TRUE)

setwd("./Step4.IterativeCleaning/CleanedClusters.R1")
Files <- list.files()

# load files
SeuratList <- lapply(Files, readRDS)
names(SeuratList) <- gsub(".rds", "", Files)
  
  
## Define instance
# define instance (data preparation)
subsample <- function(data, job, i) {
  MajorCluster <- names(data)
...
HarmoneyRecluster <- function(data, job, instance) { # instance is inherited from the last step
  ...
  data <- data %>% RunHarmony("batch", plot_convergence = TRUE)
  ...
  #                       showWarnings = FALSE)
  SaveDir <- "./Step4.IterativeCleaning/Recluster.R2"
